    ...
    return this.getUrlPath()
      .then((urlPath) => {
        const requests: Promise<SendResponse>[] = copy.map(async (message) => {
          ...
          try {
            return await this.messagingRequestHandler
              .invokeHttpRequestHandlerForSendResponse(FCM_SEND_HOST, urlPath, request);
          } catch (error) {
            // Catch errors and format them for the response
            return { success: false, error };
          }
        });

        // Use Promise.all and ensure we handle rejected promises manually in .catch()
